

<div align="center">

# FCSplash 0.3.0
<img width="800" height="450" alt="FCSplash 0 3 0" src="https://github.com/user-attachments/assets/e29d83e5-16b3-481e-907b-2e07f60509fd" />
<p>
  <img src="https://img.shields.io/badge/status-beta-orange" alt="Status Release">
  <img src="https://img.shields.io/badge/version-v0.2.0-blue" alt="Version 0.3.0">
</p>

A Beat Saber mod that adds a splash animation at the end if you FC a song with custom picture support (now featuring procedural rounded corners!).

</div>

---

## Usage

1. Launch Beat Saber with Mod installed.
2. Place 1 or more images in "UserData/FCSplash/Images & Gifs"
3. [OPTIONAL] Add your own custom sound effect to play when the FCSplash appears
4. Configure your settings in UserData/FCSplash.json
3. Full Combo (FC) a song.

---

## To-Do List

- [ ] Add settings UI
- [x] Gif support
- [x] Change the text
- [x] Change scale
- [ ] Add a preview in the settings UI
- [x] Choose different animation styles
- [x] Change position
- [x] Audio integration
- [ ] Add fade transitions
- [x] Optional effects
- [x] Unload textures for RAM usage
- [x] Rounded corners for images and gifs
